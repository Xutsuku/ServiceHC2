# -*- coding: utf-8 -*-
"""
Created on Mon Feb 12 14:11:20 2018
...
en = dataMerge('EN',False,SL_hour,Staff_hour,queue_hour)
en.columns
en.shape
...
SL_hour = read_excel(path,sheetname='SL_hour')
Staff_hour = read_excel(path,sheetname='Staff_hour')
queue_hour = read_excel(path,sheetname='queue_hour')
#deal_hour = read_excel(path,sheetname='deal_hour') 

data = en[en['product'] =='酒店']
data = hk[hk['product'] =='机票']

fea_cols = ['totalcallincount', 'avgtalktime',
       'eidno', 'avgQueue']

fea_cols = ['totalcallincount','ConnectedInTwenty', 
       ]

print ('------- the origin analysis-----------')    
sns.pairplot(data,x_vars=fea_cols,y_vars ='eidno',size=7,aspect=0.4,kind='reg')
